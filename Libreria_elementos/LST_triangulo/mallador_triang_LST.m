...
%1 - 4 - 2 
    ...
    disX = linspace(-1,1,2*divX-1);
    disY = linspace(-1,1,2*divY-1);
    
    nnod = (2*divX-1)*(2*divY-1);
    nodos = zeros(nnod,2);
    ...
    padding = 2*divY-1;%el mismo de la siguiente columna
    for i = 1:divX-1
        offset_elems = (i-1)*(divY-1)*2;
        offset = (i-1)*2*(2*divY-1);
        for j = 1:divY-1
            inicial = offset+2*j-1;
            elems(offset_elems+2*j-1,:) = [inicial inicial+2+2*padding inicial+2 inicial+padding+1 inicial+padding+2 inicial+1];%los que se ven para arriba
            elems(offset_elems+2*j,:) = [inicial inicial+2*padding inicial+2*padding+2 inicial+padding inicial+2*padding+1 inicial+padding+1];%los que se ven para abajo
        end
    end
