classdef MindlinQ4 < handle
    ...
    methods
        function mesh = MindlinQ4(bordes,divx,divy,E,v,t)
            dofselem = 6;
        ...
        function mesh = armar_R(mesh,carga_v)
            for i=1:mesh.counts.nelem
                nodoid = mesh.elems(i,:);
        ...
        function [esfuerzos] = esfuerzo(mesh)
            ...
            v = mesh.material.v;
            t = mesh.material.t;
            esfuerzos = zeros(nelem,8); %Nx, Ny,Nxy, Mx, My, Mxy, Qx, Qy
            for i = 1:nelem
                nodoid = mesh.elems(i,:);
                ...
                [membrana, momento] = stress_MQ4(Coord, Uel, E,v,t);
                esfuerzos(i,1:3) = membrana;
                esfuerzos(i,4:8) = momento;
            end
        end
