...
[mesh.nodos, mesh.elems.con, transformada] = crearMesh(Elem, Nodos, divisiones);

nnod = size(mesh.nodos,1); %cant de nodos
ndof = 6*nnod; %degrees of freedom
nelem = size(mesh.elems.con,1); %cantidad de elementos
...
R(mesh.dofs(posL,:)) = Carga;


%BC      Solo giro en X para E F G H                    empotramientos para K y J2
BC = [reshape(mesh.dofs(transformada(5:8),[1:3 5:6])', 1, []) mesh.dofs(transformada(11), :) mesh.dofs(transformada(15), :)];
...
for i= 1:12
    ...
    nodo1 = mesh.nodos(nodo1ID,:);
    nodo2 = mesh.nodos(nodo2ID,:);
    L = norm(nodo2-nodo1);
...
for i = 1:12
    ...
    for j = (inicioElem(i) + cantidad)
        ...
        if flag_no_deformada
            ...
            Yplt = mesh.nodos([nodo1ID, nodo2ID],2);
            Zplt = mesh.nodos([nodo1ID, nodo2ID],3);
        else
...
for ifrecuencias = 1:3
    ...
    Ufrecuencias(mesh.dofs(transformada(13:15),4)) = []; %eliminar doble giros
...
for ipandeo = 1:3
    ...
    Upandeo(mesh.dofs(transformada(13:15),4)) = []; %eliminar doble giros
...
dofs = mesh.dofs(mesh.elems.con(inicioElem(10),1),:);
movL = U(dofs);
...
function [Kel, Mel] = K_torsion(seccion, material, L)
    Kel = zeros(12);
    Mel = zeros(12);
